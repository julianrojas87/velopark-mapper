...
async function run() {
    ...
    const kml = await readFile('data/oostende/oostende.kml', 'utf8');
    ...
    for (const folder of json) {
        if (Array.isArray(folder.Placemark)) {
            for (const p of folder.Placemark) {
                await map(p, appProfile);
            }
        } else {
            await map(folder.Placemark, appProfile);
        }
    }
...
async function map(p, jsonLD) {
    let name = null;
    if (p.name._text) {
        name = p.name._text.replace(/\s/g, '-').replace(/\//g, '-');
    } else {
        name = p.name._cdata.replace(/\s/g, '-').replace(/\//g, '-');
    }
    ...
    const photo = p.description._cdata.split('"')[1];
    const capacity = parseInt(p.description._cdata.split('<br><br>')[1].split(' ')[0]);

    jsonLD['@id'] = 'https://velopark.ilabt.imec.be/data/Stad-Oostende_' + name;
    ...
    jsonLD.ownedBy.companyName = 'Stad-Oostende';
    jsonLD.ownedBy['@id'] = `https://www.oostende.be/`;
    jsonLD.operatedBy.companyName = 'Stad-Oostende';
    jsonLD.operatedBy['@id'] = `https://www.oostende.be/`;
    ...
    jsonLD['contactPoint']['email'] = 'stadsbestuur@oostende.be';
    jsonLD['contactPoint']['telephone'] = '059805500';
    jsonLD['photos'][0]['image'] = photo;
    ...
    const fileName = `Stad-Oostende_${name}.jsonld`;
